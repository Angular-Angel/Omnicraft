...
import net.angle.omnicraft.client.Client;
...
public abstract class Block {
    ...
    public void bufferTop(VertexBuffer buffer, Vec3 vPos, Vec2 vTexCoord, float x, float y, float z) {
        bufferFlatVertices(buffer, vPos, vTexCoord, x, y, z - 1, -1, 0, 1);
    }
    
    public void bufferBottom(VertexBuffer buffer, Vec3 vPos, Vec2 vTexCoord, float x, float y, float z) {
        bufferFlatVertices(buffer, vPos, vTexCoord, x, y - 1, z, -1, 0, -1);
    }
    
    public void bufferFront(VertexBuffer buffer, Vec3 vPos, Vec2 vTexCoord, float x, float y, float z) {
        bufferFlatVertices(buffer, vPos, vTexCoord, x, y, z, -1, -1, 0);
    }
    
    public void bufferBack(VertexBuffer buffer, Vec3 vPos, Vec2 vTexCoord, float x, float y, float z) {
        bufferFlatVertices(buffer, vPos, vTexCoord, x - 1, y, z - 1, 1, -1, 0);
    }
    
    public void bufferLeftSide(VertexBuffer buffer, Vec3 vPos, Vec2 vTexCoord, float x, float y, float z) {
        bufferFlatVertices(buffer, vPos, vTexCoord, x - 1, y, z, 0, -1, -1);
    }
    
    public void bufferRightSide(VertexBuffer buffer, Vec3 vPos, Vec2 vTexCoord, float x, float y, float z) {
        bufferFlatVertices(buffer, vPos, vTexCoord, x, y, z - 1, 0, -1, 1);
    }
    ...
    public void bufferVertices(VertexBuffer buffer, Vec3 vPos, Vec2 vTexCoord, Chunk chunk, int blockx, int blocky, int blockz) {
        ...
        if (topIsVisible(chunk, blockx, blocky, blockz))
            bufferTop(buffer, vPos, vTexCoord, chunk.x + blockx, chunk.y + blocky, chunk.z + blockz);
        
        //Draw bottom:
        if (bottomIsVisible(chunk, blockx, blocky, blockz))
            bufferBottom(buffer, vPos, vTexCoord, chunk.x + blockx, chunk.y + blocky, chunk.z + blockz);
        
        //Draw front
        if (frontIsVisible(chunk, blockx, blocky, blockz))
            bufferFront(buffer, vPos, vTexCoord, chunk.x + blockx, chunk.y + blocky, chunk.z + blockz);
        
        //Draw back
        if (backIsVisible(chunk, blockx, blocky, blockz))
            bufferBack(buffer, vPos, vTexCoord, chunk.x + blockx, chunk.y + blocky, chunk.z + blockz);
        
        //Draw left
        if (leftSideIsVisible(chunk, blockx, blocky, blockz))
            bufferLeftSide(buffer, vPos, vTexCoord, chunk.x + blockx, chunk.y + blocky, chunk.z + blockz);
        
        //Draw right
        if (rightSideIsVisible(chunk, blockx, blocky, blockz))
            bufferRightSide(buffer, vPos, vTexCoord, chunk.x + blockx, chunk.y + blocky, chunk.z + blockz);
    }
