...
public class SimplexNoise {  // Simplex noise in 2D, 3D and 4D
  ...
  private static final Vec3d[] vectors = {new Vec3d(1, 1, 0), new Vec3d(-1, 1, 0), new Vec3d(1, -1, 0), new Vec3d(-1, -1, 0),
  ...
  public SimplexNoise(int seed) {
    ...
    if (seed == RANDOMSEED){
        OmniRandom rand = new OmniRandom();
        seed=rand.nextInt();
    }
    
    //the random for the swaps
    OmniRandom rand = new OmniRandom(seed);
  ...
  private static double dot(Vec3d g, double x, double y) {
    return g.x * x + g.y * y; }
  ...
  public double noise(double xin, double yin) {
    ...
    double t = (i + j) * G2;
    double X0 = i - t, Y0 = j - t; // Unskew the cell origin back to (x,y) space
    double x0 = xin - X0, y0 = yin - Y0; // The x,y distances from the cell origin
    ...
    if(t0 < 0) 
      ...
      t0 *= t0;
      n0 = t0 * t0 * dot(vectors[gi0], x0, y0);  // (x,y) of grad3 used for 2D gradient
    }
    ...
    if(t1 < 0) 
      ...
      t1 *= t1;
      n1 = t1 * t1 * dot(vectors[gi1], x1, y1);
    }
    ...
    if(t2 < 0) 
      ...
      t2 *= t2;
      n2 = t2 * t2 * dot(vectors[gi2], x2, y2);
    }
